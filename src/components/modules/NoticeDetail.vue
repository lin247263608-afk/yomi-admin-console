...
<template>
  <ModalDialog :title="`公告详情 · ${row.id}`" eyebrow="NOTICE DETAIL" size="wide" @close="emit('close')">
    <section class="notice-detail-hero">
      ...
      <div><small>{{ row.channel }}</small><h3>{{ row.title }}</h3><p>{{ row.subtitle }}</p></div>
    ...
    <div class="notice-detail-grid">
      ...
      <div class="notice-detail-grid__wide"><span>公告标题</span><strong>{{ row.title }}</strong></div>
      <div class="notice-detail-grid__wide"><span>副标题</span><strong>{{ row.subtitle }}</strong></div>
    ...
    <section class="notice-content-preview">
      ...
      <div class="notice-content-preview__body"><p v-for="(paragraph, index) in String(row.content ?? '').split('\n').filter(Boolean)" :key="index">{{ paragraph }}</p></div>
    </section>
